...
function SingleWorkComponent( props) {
     ...
     <div data-aos-duration="2000" data-aos="fade-up"  data-aos-anchor-placement="bottom" className='w-[5px] rounded-[0px] bg-black'></div>      
           <div onClick={()=>{activeTabId!==data?.workId?setActiveTabId(data?.workId):setActiveTabId("none")}} data-aos="fade-up" data-aos-duration="1000"   data-aos-anchor-placement="bottom" className="flex flex-col w-full p-3 cursor-pointer md:p-4">
            <div className='flex flex-row items-center justify-between '>
                ...
                <div className='cursor-pointer' title={activeTabId===data?.workId?'Collapse':'Expand'}  onClick={()=>{activeTabId!==data?.workId?setActiveTabId(data?.workId):setActiveTabId("none")}}>
            ...
            </div>
            {
                ...
                {
                    data?.workLink?
                    <a target='_blank' className=" text-[14px] md:text-[16px] mt-2 hover:text-[#020268] w-fit underline underline-offset-4 font-semibold flex flex-row items-center" href={data?.workLink}>
                        View More
                    </a>:null
                   }
                </div>
